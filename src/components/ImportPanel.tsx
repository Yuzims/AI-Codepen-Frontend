...
const ImportPanel: React.FC<ImportPanelProps> = ({
    ...
    const availablePens = (userPens || [])
        .filter((pen): pen is Pen => Boolean(pen) && typeof pen.id === 'string')
        .filter(pen => !currentPenId || pen.id !== currentPenId);
    ...
    const importAllCss = () => {
        const availableCssIds = availablePens.filter(pen => pen.css.trim()).map(pen => pen.id);
    ...
    const importAllJs = () => {
        const availableJsIds = availablePens.filter(pen => pen.js.trim()).map(pen => pen.id);
